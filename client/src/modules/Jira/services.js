...
const filterIssues = (issues: ?Object[], year: number) => {
    ...
    for(let i = 0; i < issues.length; i += 1){
        ...
        if(!(["Rejected", "Done"].includes(issues[i].fields.status.name)) && (date.getFullYear() === year)){
...
export const generateChartData = (projectKey: string, year: number, timeInterval: TimeInterval) => {
    let issuesPromise;
    let xLabels;
